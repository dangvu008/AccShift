...
export const getCurrentApiKey = async () => {
  try {
    ...
    const apiStateJson = await AsyncStorage.getItem(
    ...
    try {
      const storedKeysJson = await AsyncStorage.getItem(
        STORAGE_KEYS.WEATHER_API_KEYS
      )
      if (storedKeysJson) {
 ...
 * @returns {Promise<Object|null>} Dữ liệu thời tiết từ cache hoặc null
 */
export const handleWeatherError = async (error, type = 'current') => {
  ...
  // Nếu không có cache, trả về null
  console.log(
    `Không có cache, không thể hiển thị dữ liệu thời tiết cho ${type}`
  )
  return null
}
